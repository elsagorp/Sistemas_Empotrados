...
char txbuffer_ISR[200], dataCMD_ISR[200];
...
//Método de duardado de comandos
//const char cmd1[] = {"set MIC2_ledgreen 1"};
const char cmd1[] = {"set ledgreen"};
   ...
   // LED_VERDE = LED_OFF_STATE;
    ...
    while(1){
       ...
       if(comando_detectado){
                ...
                if ( !strcmp ( ((const char*)dataCMD_ISR), cmd3) ) LATAbits.LATA1 = 1;
                if ( !strcmp ( ((const char*)dataCMD_ISR), cmd4) ) LATAbits.LATA1 = 0;
       ...
       //(Allowprint)&&
     if((BufferLoadDone)&&(U1STAbits.TRMT)){   // Si se cargaron todos los datos al buffer U1TXREG y finalizó la última transmisión...
    
        if(U1_PrintRate_ISR ++ >=100){        //Determinar el ritmo de transmisión de datos de la Uart1 TX
            if(contador == 100) contador = 0;
            if(contador <= 100){
                memset(txbuffer_ISR,'\0',sizeof(txbuffer_ISR)); // Clear buffer and fill in with NULL
...
void __attribute__ ((__interrupt__, no_auto_psv)) _U1TXInterrupt (void){ // Actualmente configurado para U1STAbits.UTXISEL=0
        ...
        if(!U1STAbits.UTXBF){      // Mientras el buffer de transmisión NO se encuentre completo, continuar cargando el
        ...
        if (nextchar == strlen(txbuffer_ISR)){ // Si se ha finalizado la transmision de todos los caracteres --› Deshabilitar inter
            LATAbits.LATA0 = !PORTAbits.RA0;  //Visualmente ver si sigue transmitioendo o no
            nextchar = 0;
...
void __attribute__((__interrupt__, no_auto_psv))_U1RXInterrupt (void){
        if (comando_detectado == 0){
                ...
                data_count++;
                if ((dataCMD_ISR[data_count - 1] == 13)  || (data_count >= 20)) // Retorno de carro-Intro detectad
                {
...
void __attribute__((__interrupt__, no_auto_psv))_U1RXInterrupt (void){
        if (comando_detectado == 0 && comando == 0){
                dataCMD_ISR[data_count] = U1RXREG; // Obtener caracter recibido en el buffer
                data_count++;
                if ((dataCMD_ISR[data_count - 1] == 13) || (data_count >= 25)) // Retorno de carro-Intro detectad
                {   //|| (dataCMD_ISR[data_count - 1] == 10)
                        // Eliminar retorno de carro del str
                        dataCMD_ISR[data_count - 1] = '\0';
                        comando_detectado = 1;
                        data_count = 0;
                }
        }else if(comando_detectado == 0 && comando == 1){
                dataCMD_ISR[data_count] = U1RXREG; // Obtener caracter recibido en el buffer
                data_count++;
                if ((dataCMD_ISR[data_count - 1] == 65) || (data_count >= 25)){ // Retorno de carro-Intro detectad
