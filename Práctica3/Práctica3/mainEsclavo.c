...
unsigned char Stoprint = 0;

char txbuffer_ISR[200], dataCMD_ISR[50]; 

unsigned int contador = 0, flag_sync=0;
...
const char cmd11[] = {"Pª"} ;;            //0x50 0x01 0xAA 
const char cmd22[] = {'P', NULL, 'ª'} ;;   //0x50 0x00 0xAA 
const char cmd33[] = {"Qª"} ;;            //0x51 0x01 0xAA 
const char cmd44[] = {'Q', NULL, 'ª'} ;;   //0x51 0x00 0xAA 
const char cmd55[] = {"Rª"} ;             //0x52 0x01 0xAA 
const char cmd66[] = {'R', NULL, 'ª'} ;;   //0x52 0x00 0xAA 
   ...
   LATAbits.LATA1 = 0;
    ...
    if(flag_sync ==1){    
     if(comando_detectado){
                 ...
                 //Caso del esclavo con sus comandos correspondientes
                if ( !strcmp ( ((const char*)dataCMD_ISR), cmd11) ) LATAbits.LATA0 = 1;
                else if ( !strcmp ( ((const char*)dataCMD_ISR), cmd22) ) LATAbits.LATA0 = 0;
                else if ( !strcmp ( ((const char*)dataCMD_ISR), cmd33) ) LATAbits.LATA1 = 1;
                else if ( !strcmp ( ((const char*)dataCMD_ISR), cmd44) ) LATAbits.LATA1 = 0;
                else if ( !strcmp (((const char*) dataCMD_ISR), cmd55) ) Allowprint = 1;
                else if ( !strcmp (((const char* )dataCMD_ISR), cmd66) ){
                    Stoprint = 1;
                    Allowprint = 0;
                }else{
    ...
    if((Allowprint)&&(BufferLoadDone)&&(U1STAbits.TRMT)){   // Si se cargaron todos los datos al buffer U1TXREG y finalizó la última transmisión...
            ...
            if(U1_PrintRate_ISR ++ >=4){        //Determinar el ritmo de transmisión de datos de la Uart1 TX
                if(contador <= 100){
     ...
     // Detener impresión
    if((Stoprint)&&(BufferLoadDone)&&(U1STAbits.TRMT)){
        
                memset(txbuffer_ISR,'\0',sizeof(txbuffer_ISR));
                sprintf (txbuffer_ISR, "Impresion de datos detenida  \n") ;
                Stoprint = 0;
                BufferLoadDone = 0;
                if(U1STAbits.UTXBF)IFS0bits.U1TXIF = 0; //Reseteo el flag de transmisión ISR
                asm("nop");
